...
/* Created on:     20/11/2023 6:32:45 p.�m.                     */
/*==============================================================*/
...
create table DIRECCION (
   ...
   POSICION_COMP        NUMBER(2)             not null,
   IDTIPOPERSONA        VARCHAR2(2)           not null,
   IDTIPODOC            VARCHAR2(2)           not null,
   NDOCUMENTO           VARCHAR2(12)          not null,
   IDNOM                VARCHAR2(5)           not null,
   constraint PK_DIRECCION primary key (IDDIRECCION, POSICION_COMP)
);
...
create table NOMENCLATURA (
   ...
   constraint PK_NOMENCLATURA primary key (IDNOM)
);
...
create table TIPODOC (
   IDTIPODOC            VARCHAR2(2)           not null,
   DESCTIPODOC          VARCHAR2(25)          not null,
...
alter table DIRECCION
   add constraint FK_DIRECCIO_RELATIONS_COMPONEN foreign key (POSICION_COMP)
      references COMPONENTEDIRECC (POSICION);

alter table DIRECCION
   add constraint FK_DIRECCIO_RELATIONS_NOMENCLA foreign key (IDNOM)
      references NOMENCLATURA (IDNOM);
...
create sequence BASE_PERSONAS03;